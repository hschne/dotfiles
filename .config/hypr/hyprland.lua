...
-- Applications
hl.bind(mainMod .. " + X", hl.dsp.exec_cmd(terminal .. " " .. editor .. " ~/Documents/Wiki/todo.txt"), { description = "Todo" })
...
hl.bind(mainMod .. " + CTRL + SHIFT + Left",  hl.dsp.workspace.move({ monitor = "l" }), { description = "Move workspace to left monitor" })
hl.bind(mainMod .. " + CTRL + SHIFT + Right", hl.dsp.workspace.move({ monitor = "r" }), { description = "Move workspace to right monitor" })
...
hl.bind(mainMod .. " + Space",         hl.dsp.window.cycle_next({ floating = true }), { description = "Cycle floating windows" })

-- Config management
hl.bind(mainMod .. " + SHIFT + W", hl.dsp.exec_cmd("pkill waybar; uwsm-app -- waybar"), { description = "Restart Waybar" })
hl.bind(mainMod .. " + R",         hl.dsp.exec_cmd("hyprctl reload"),                   { description = "Reload Hyprland" })
...
hl.bind(mainMod .. " + SHIFT + Print", hl.dsp.exec_cmd([[grim -g "$(slurp)" - | wl-copy]]), { description = "Screenshot region to clipboard" })

-- Screen recording
hl.bind(mainMod .. " + SHIFT + R", hl.dsp.exec_cmd("~/.scripts/screen-record --silent"), { description = "Toggle screen recording (silent)" })
...
hl.bind(mainMod .. " + V", hl.dsp.exec_cmd("voxtype record stop"),  { release = true, description = "Voxtype stop recording" })
...
hl.bind("XF86AudioRaiseVolume", hl.dsp.exec_cmd("swayosd-client --output-volume raise"),      { repeating = true, locked = true, description = "Volume up" })
hl.bind("XF86AudioLowerVolume", hl.dsp.exec_cmd("swayosd-client --output-volume lower"),      { repeating = true, locked = true, description = "Volume down" })
hl.bind("XF86AudioMute",        hl.dsp.exec_cmd("swayosd-client --output-volume mute-toggle"), { repeating = true, locked = true, description = "Toggle mute" })
hl.bind("XF86AudioMicMute",     hl.dsp.exec_cmd("swayosd-client --input-volume mute-toggle"),  { repeating = true, locked = true, description = "Toggle mic mute" })

-- Brightness (using swayosd for visual feedback)
hl.bind("XF86MonBrightnessUp",   hl.dsp.exec_cmd("swayosd-client --brightness raise"), { repeating = true, locked = true, description = "Brightness up" })
hl.bind("XF86MonBrightnessDown", hl.dsp.exec_cmd("swayosd-client --brightness lower"), { repeating = true, locked = true, description = "Brightness down" })
...
hl.bind(mainMod .. " + CTRL + comma",        hl.dsp.exec_cmd([[makoctl mode -t do-not-disturb && makoctl mode | grep -q 'do-not-disturb' && notify-send "Silenced notifications" || notify-send "Enabled notifications"]]), { description = "Toggle silencing notifications" })
...
hl.bind("SHIFT + ALT + period", hl.dsp.pass({ window = "class:^(com\\.obsproject\\.Studio)$" }), { description = "OBS Zoom Toggle" })
hl.bind("SHIFT + ALT + s",      hl.dsp.pass({ window = "class:^(com\\.obsproject\\.Studio)$" }), { description = "OBS Start Recording" })
hl.bind("SHIFT + ALT + o",      hl.dsp.pass({ window = "class:^(com\\.obsproject\\.Studio)$" }), { description = "OBS Stop Recording" })
...
hl.define_submap("resize", function()
    ...
    hl.bind("Left",  hl.dsp.window.resize({ x = -20, y = 0,   relative = true }), { repeating = true, description = "Resize left" })
    hl.bind("Down",  hl.dsp.window.resize({ x = 0,   y = 20,  relative = true }), { repeating = true, description = "Resize down" })
    hl.bind("Up",    hl.dsp.window.resize({ x = 0,   y = -20, relative = true }), { repeating = true, description = "Resize up" })
    hl.bind("Right", hl.dsp.window.resize({ x = 20,  y = 0,   relative = true }), { repeating = true, description = "Resize right" })
    hl.bind("Return", hl.dsp.submap("reset"))
    hl.bind("Escape", hl.dsp.submap("reset"))
